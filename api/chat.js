...
module.exports = async function handler(req, res) {
  ...
  if (!apiKey) {
    res.status(500).json({ reply: 'Server is missing GEMINI_API_KEY — set it in Vercel project settings.' });
    return;
  }

  let userMessage = (req.body && req.body.message) ? String(req.body.message).trim() : '';
  if (userMessage === '') {
    res.status(200).json({ reply: 'Ask me anything about Shreya — her projects, skills, or experience.' });
    return;
  }
  if (userMessage.length > 600) userMessage = userMessage.slice(0, 600);
